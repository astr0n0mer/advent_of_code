...
for (let i of INPUT) {
  const forward_matches = i.match(
    /([1-9]|(one)|(two)|(three)|(four)|(five)|(six)|(seven)|(eight)|(nine))/g
  );

  const backward_matches = i
    .split("")
    .reverse()
    .join("")
    .match(
      /([1-9]|(enin)|(thgie)|(neves)|(xis)|(evif)|(ruof)|(eerht)|(owt)|(eno))/g
    );

  let ten = forward_matches[0];
  let unit = backward_matches[0];

  ten = isNaN(Number(ten)) ? word_to_number_map[ten] : Number(ten);
  tens += ten;

  unit = isNaN(Number(unit))
    ? word_to_number_map[unit.split("").reverse().join("")]
    : Number(unit);
  units += unit;
}

console.log(tens * 10 + units);
