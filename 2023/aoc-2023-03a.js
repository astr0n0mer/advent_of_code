...
const finalSumOfParts = INPUT.reduce((prevLineSumOfParts, line, lineIndex) => {
  const numberMatches = line.matchAll(/\d+/g);
  //   console.log({ line });

  const numberMatchesFiltered = Array.from(numberMatches).filter((match) => {
    const startIndex = match.index;
    const endIndex = startIndex + match[0].length - 1;
    const safeStartIndex = startIndex - 1 >= 0 ? startIndex - 1 : startIndex;
    const safeEndIndex =
      endIndex + 1 <= line.length - 1 ? endIndex + 1 : endIndex;
    ...
    if (
      hasSymbol(line[safeStartIndex]) ||
      hasSymbol(line[safeEndIndex]) ||
      (lineIndex - 1 >= 0 &&
        hasSymbol(
          INPUT[lineIndex - 1].slice(safeStartIndex, safeEndIndex + 1)
        )) ||
      (lineIndex + 1 <= INPUT.length - 1 &&
        hasSymbol(INPUT[lineIndex + 1].slice(safeStartIndex, safeEndIndex + 1)))
    ) {
      return true;
    }
  ...
  //   const numbers = numberMatchesFiltered.map((match) => Number(match[0]));
  //   console.log({ numbers });

  //   console.log(numbers.length > 0 ? { numbers } : "");
  const sumOfParts = numberMatchesFiltered.reduce(
    (prevSum, numberMatch) => prevSum + Number(numberMatch[0]),
    0
  );
  //   console.log({ sumOfParts });
  return prevLineSumOfParts + sumOfParts;
}, 0);

console.log({ finalSumOfParts });
