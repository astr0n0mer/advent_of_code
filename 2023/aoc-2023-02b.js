...
const MAX_GREEN = 13;
const MAX_BLUE = 14;
let sum_of_power_of_games = 0;

for (let input of INPUT) {
  const draws = input.split(": ")[1].split("; ");
  let max_red_reqd = 0;
  let max_green_reqd = 0;
  let max_blue_reqd = 0;

  for (let draw of draws) {
    const red_balls = draw.match(/(\d+) red/)
      ? Number(draw.match(/(\d+) red/)[1])
      : 0;
    const green_balls = draw.match(/(\d+) green/)
      ? Number(draw.match(/(\d+) green/)[1])
      : 0;
    const blue_balls = draw.match(/(\d+) blue/)
      ? Number(draw.match(/(\d+) blue/)[1])
      : 0;

    max_red_reqd = Math.max(max_red_reqd, red_balls);
    max_green_reqd = Math.max(max_green_reqd, green_balls);
    max_blue_reqd = Math.max(max_blue_reqd, blue_balls);
  }

  power_of_game = max_red_reqd * max_green_reqd * max_blue_reqd;
  sum_of_power_of_games += power_of_game;
}

console.log({ sum_of_power_of_games });
