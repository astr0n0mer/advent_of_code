...
const finalGearRatio = INPUT.reduce((prevLineGearRatio, line, lineIndex) => {
  // console.log({ line });
  const starMatches = line.matchAll(/\*/g);

  const lineGearRatio = Array.from(starMatches).reduce(
    (prevStarGearRatio, starMatch) => {
      const starStartIndex = starMatch.index;
      const starEndIndex = starStartIndex + starMatch[0].length - 1;
      const validNeighbors = [];
      // console.log(`Star match`, { starMatch, starStartIndex, starEndIndex });

      const leftMatch = line.slice(0, starEndIndex + 1).match(/\d+\*$/);
      if (leftMatch) {
        validNeighbors.push(
          Number(leftMatch[0].slice(0, leftMatch[0].length - 1))
        );
        // console.log(`leftMatch ${leftMatch[0]} is valid`);
      } else {
        // console.log(`No leftMatch XXXXXXXXX`);
      }

      const rightMatch = line.slice(starEndIndex).match(/^\*\d+/);
      if (rightMatch) {
        validNeighbors.push(Number(rightMatch[0].slice(1)));
        // console.log(`rightMatch ${rightMatch[0]} is valid`);
      } else {
        // console.log(`No rightMatch XXXXXXXXX`);
      }

      if (lineIndex - 1 >= 0) {
        const topMatches = INPUT[lineIndex - 1].matchAll(/\d+/g);
        Array.from(topMatches).forEach((topMatch) => {
          // console.log({ topMatch: topMatch[0] });
          const numberStartIndex = topMatch.index;
          const numberEndIndex = numberStartIndex + topMatch[0].length - 1;
          // console.log({ numberStartIndex, numberEndIndex });
          // console.log({ starStartIndex, starEndIndex });
          if (
            (numberStartIndex >= starStartIndex - 1 &&
              numberStartIndex <= starEndIndex + 1) ||
            (numberEndIndex >= starStartIndex - 1 &&
              numberEndIndex <= starEndIndex + 1)
          ) {
            // console.log(`topMatch ${topMatch[0]} is valid`);
            validNeighbors.push(Number(topMatch[0]));
          } else {
            // console.log(`topMatch ${topMatch[0]} is XXXXXXXXX`);
          }
        });
      }

      if (lineIndex + 1 <= INPUT.length - 1) {
        const bottomMatches = INPUT[lineIndex + 1].matchAll(/\d+/g);
        Array.from(bottomMatches).forEach((bottomMatch) => {
          // console.log({ bottomMatch: bottomMatch[0] });
          const numberStartIndex = bottomMatch.index;
          const numberEndIndex = numberStartIndex + bottomMatch[0].length - 1;
          // console.log({ numberStartIndex, numberEndIndex });
          // console.log({ starStartIndex, starEndIndex });
          if (
            (numberStartIndex >= starStartIndex - 1 &&
            ...
            // console.log(`bottomMatch ${bottomMatch[0]} is valid`);
            validNeighbors.push(Number(bottomMatch[0]));
          } else {
            // console.log(`bottomMatch ${bottomMatch[0]} is XXXXXXXXX`);
          }
        });
      }

      // console.log({ validNeighbors });
      const gearRatio = validNeighbors[0] * validNeighbors[1];

      if (validNeighbors.length == 2) {
        return prevStarGearRatio + gearRatio;
      }

      return prevStarGearRatio;
    },
    0
  );

  // console.log({ lineIndex, lineGearRatio });

  return prevLineGearRatio + lineGearRatio;
}, 0);

console.log({ finalGearRatio });
